...
public sealed class PowerShellWrapperGenerator : IIncrementalGenerator
{
        ...
        string commandName)
    {
        try
        {
            ...
            var parameters = discoveredCommand.Parameters
                ...
                .OrderBy(x => x.Name, StringComparer.Ordinal)
                .Select(x => CreateParameterModel(
                    context.SemanticModel.Compilation,
                    x.Name,
                    x.ParameterType,
                    x.AcceptsPipelineValue,
                    x.AcceptsPipelinePropertyName))
                .ToList();
        ...
        for (var current = cmdletType; current is not null; current = current.BaseType)
        {
            foreach (var property in current.GetMembers().OfType<IPropertySymbol>())
            {
                ...
                yield return new ParameterModel(
                    ...
                    AcceptsPipelineValue(property),
                    AcceptsPipelinePropertyName(property));
            }
        }
    }
    ...
    private static ParameterModel CreateParameterModel(
        ...
        Type parameterType,
        bool acceptsPipelineValue,
        bool acceptsPipelinePropertyName)
    {
        ...
        return new ParameterModel(
            ...
            isOptionalValueType,
            acceptsPipelineValue,
            acceptsPipelinePropertyName);
    }
...
foreach ($parameter in $command.Parameters.Values | Sort-Object Name) {{
    ...
    [Console]::Out.WriteLine(($parameter.Name + '=' + $parameterType + ';' + [int]$parameter.ValueFromPipeline + ';' + [int]$parameter.ValueFromPipelineByPropertyName))
}}
...
";
        ...
        foreach (var line in lines.Skip(1))
        {
            ...
            var segments = line.Substring(separatorIndex + 1).Split(';');
            var assemblyQualifiedTypeName = segments[0];
            var parameterType = Type.GetType(assemblyQualifiedTypeName, throwOnError: false) ?? typeof(object);
            var acceptsPipelineValue = segments.Length > 1 && segments[1] == "1";
            var acceptsPipelinePropertyName = segments.Length > 2 && segments[2] == "1";
            parameters.Add(new DiscoveredParameter(name, parameterType, acceptsPipelineValue, acceptsPipelinePropertyName));
        }
    ...
    private static string RenderSource(
        ...
        bool useTypedInvoke)
    {
        var pipelineParameters = parameters
            .Where(static parameter => parameter.AcceptsPipelineValue || parameter.AcceptsPipelinePropertyName)
            .ToList();
        ...
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    public static global::System.Collections.Generic.IReadOnlyList<global::PowerInvoke.PowerShellPipelineParameter> PipelineParameters { get; } =");
        if (pipelineParameters.Count == 0)
        {
            builder.AppendLine("        global::System.Array.Empty<global::PowerInvoke.PowerShellPipelineParameter>();");
        }
        else
        {
            builder.AppendLine("        new global::PowerInvoke.PowerShellPipelineParameter[]");
            builder.AppendLine("        {");
            foreach (var parameter in pipelineParameters)
            {
                builder.Append("            new global::PowerInvoke.PowerShellPipelineParameter(\"")
                    .Append(parameter.Name)
                    .Append("\", \"")
                    .Append(EscapeStringLiteral(parameter.TypeName))
                    .Append("\", ")
                    .Append(parameter.AcceptsPipelineValue ? "true" : "false")
                    .Append(", ")
                    .Append(parameter.AcceptsPipelinePropertyName ? "true" : "false")
                    .AppendLine("),");
            }
        ...
        builder.AppendLine(")");
        builder.AppendLine("    {");
        RenderParameterListBuilder(builder, parameters);
        RenderInvoke(builder, commandName, returnTypeName, useTypedInvoke, "parameters", "null");
        builder.AppendLine("    }");
        ...
        if (pipelineParameters.Count > 0)
        {
            ...
            builder.Append("    public global::System.Collections.ObjectModel.Collection<")
                ...
                .Append("> ")
                .Append(methodName)
                .Append("<TInput>(global::System.Collections.Generic.IEnumerable<TInput> input");
            ...
            builder.AppendLine(")");
            builder.AppendLine("    {");
            builder.AppendLine("        global::System.ArgumentNullException.ThrowIfNull(input);");
            RenderParameterListBuilder(builder, parameters);
            RenderInvoke(builder, commandName, returnTypeName, useTypedInvoke, "parameters", "input");
            builder.AppendLine("    }");
        }
        builder.AppendLine("}");
    ...
    private static void RenderParameterListBuilder(StringBuilder builder, IReadOnlyList<ParameterModel> parameters)
    {
    ...
    private static void RenderInvoke(
    ...
    private static bool AcceptsPipelineValue(IPropertySymbol property)
    {
        return property.GetAttributes()
            .Where(static attribute => attribute.AttributeClass?.ToDisplayString() == ParameterAttributeName)
            .Any(GetValueFromPipeline);
    }

    private static bool AcceptsPipelinePropertyName(IPropertySymbol property)
    {
        return property.GetAttributes()
            .Where(static attribute => attribute.AttributeClass?.ToDisplayString() == ParameterAttributeName)
            .Any(GetValueFromPipelineByPropertyName);
    }

    private static bool GetValueFromPipeline(AttributeData attributeData)
    {
        return GetNamedBooleanArgument(attributeData, "ValueFromPipeline");
    }

    private static bool GetValueFromPipelineByPropertyName(AttributeData attributeData)
    {
        return GetNamedBooleanArgument(attributeData, "ValueFromPipelineByPropertyName");
    }

    private static bool GetNamedBooleanArgument(AttributeData attributeData, string name)
    {
    ...
    private sealed class ParameterModel
    {
        public ParameterModel(string name, string identifier, string typeName, bool isOptionalValueType)
            : this(name, identifier, typeName, isOptionalValueType, acceptsPipelineValue: false, acceptsPipelinePropertyName: false)
        {
        }
            ...
            bool acceptsPipelinePropertyName)
        {
            ...
            AcceptsPipelineValue = acceptsPipelineValue;
            AcceptsPipelinePropertyName = acceptsPipelinePropertyName;
        }
        ...
        public bool AcceptsPipelineValue { get; }

        public bool AcceptsPipelinePropertyName { get; }
    }
    ...
    {
        public DiscoveredParameter(string name, Type parameterType, bool acceptsPipelineValue, bool acceptsPipelinePropertyName)
        {
            Name = name;
